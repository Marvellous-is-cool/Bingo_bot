...
# Helper function to teleport a player to exit area
async def teleport_to_exit(bot, user_id):
    try:
        await bot.highrise.teleport(user_id=user_id,
                                   dest=Position(float(13), float(12),
                                               float(29)))
    except Exception as e:
        # Add fallback message if teleportation fails
        await bot.highrise.send_whisper(user_id, "Please hit the spikes in front of you to exit the playing area. Thank you!")

# Helper function to clean up player data when they leave
async def cleanup_player_data(bot, user, label=None, user_team=None):
    global player_revs, players, player_labels, ping_tasks, pinged_players
    global teams, emoji_users, player_emojis, team_emojis, auto_ping_task, round_bingos
    ...
    if user_team is not None:
        ...
        for i in range(2):  # Check both potential team member task IDs
            ...
            if task_label in ping_tasks and not ping_tasks[task_label].done():
    ...
    if label in ping_tasks and not ping_tasks[label].done():
...
async def reset_game():
    ...
    global auto_ping_task, ping_tasks, game_ended, bingo_timestamps, team_bingo_timestamps
...
async def handle_player_tip(bot, sender: "User", receiver: "User", tip: int):
    ...
    if sender.id in player_revs and sender.id in pinged_players and tip == 5:
        ...
        if player_revs[sender.id] == 0:
            ...
            await cleanup_player_data(bot, sender, label)
            
            # Teleport the player to the top level (same as gg)
            await teleport_to_exit(bot, sender.id)
            ...
            if not winner_declared:
                # If no winner yet, check for last players
                await check_for_last_players(bot)
...
async def handle_bingo_celebration(bot, user, message):
    ...
    if "bingo" in message_lower:
        ...
        if message_emoji and message_emoji == user_emoji:
            # Only process if the user hasn't already said bingo in this round
            if user.id not in round_bingos:
                ...
                # User already said bingo in this round - silently ignore
                pass
        else:
            # Emoji doesn't match - optionally notify the user
            # await bot.highrise.send_whisper(user.id, f"Your bingo emoji {message_emoji} doesn't match your registered emoji {user_emoji}. Please use {user_emoji} when saying bingo.")
            pass
...
async def handle_player_gg(bot, user, label, user_team):
    ...
    # Clean up player data
    await cleanup_player_data(bot, user, label, user_team)
    
    # Teleport the player to the top level
    await teleport_to_exit(bot, user.id)
